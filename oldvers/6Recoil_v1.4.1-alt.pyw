...
def start_moving():
    global moving
    def move_mouse():
        global moving, h_remainder, v_remainder
        if not moving:
        ...
        with open(file_path, 'r') as file:
            for line in file:
                if '=' in line:
                    ...
                    gun = gun.strip()
                    vspeed, hspeed = map(int, values.strip().split(','))
                    speed_options[gun] = {'vspeed': vspeed, 'hspeed': hspeed}
    except FileNotFoundError:
        print(f"Error: {file_name} not found.")
    except ValueError:
        print(f"Error: Invalid format in {file_name}. Format should be: gun = vspeed,hspeed")
...
speed_options = read_speed_options('speed_options_new_alt.txt')
...
def open_config_window():
    def save_and_close():
        global custom_messages, dark_mode_enabled, rf_duration, rf_interval, sleep_duration
    ...
    ttk.Label(config_window, text="Sleep Duration:").grid(row=8, column=0, padx=10, pady=5, sticky=tk.W)
    sleep_duration_entry = ttk.Entry(config_window)
    sleep_duration_entry.insert(0, str(sleep_duration))
    sleep_duration_entry.grid(row=8, column=1, padx=5, pady=5, sticky=tk.W)
    ...
    rf_duration_entry.grid(row=9, column=0, padx=5, pady=5, sticky=tk.W)
    ...
    rf_interval_entry.grid(row=9, column=1, padx=5, pady=5, sticky=tk.W)
